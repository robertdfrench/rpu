...
pub struct Core {
    ...
    /// 64 KiB of RAM. Sized so that every u16 register value is a
    /// valid byte address (max addr = u16::MAX = 65_535).
    pub memory: [u8; RAM],

    /// Any errors generated by the CPU are appended here so
    /// they can be displayed on the error console.
    pub tty: String,
